...
class QITE:
   ...
   def str_to_state(self, str_state):
      """Helper function for converting state in str form to array"""
      return str_to_state(str_state, self.num_qubits)

   def state_to_str(self, state):
      """Helper function for converting state in tuple form to str"""
...
if __name__ == '__main__':
   ...
   # Create QITE algorithm object
   qite = QITE(args, hamiltonian)

   print(qite.hamiltonian_decomp) # print hamiltonian decomposition
   print(np.linalg.eig(hamiltonian)[0].min()) # print true ground state energy
   
   # Run QITE
   energies, circuit_params = qite.run()

   # Plot final circuit generated by QITE
   qc = qite.propagate()
   qc.draw(output='mpl')
   ...
   # Simulation results
   print(energies) # energies of states over course of ITE
   plt.plot(np.arange(args.num_steps+1) * args.db, energies) # plot of energies
